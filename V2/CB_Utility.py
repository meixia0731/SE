...
modbus_slave_ip_cb_ev = "172.168.200.10"
...
def CB_SIMULATOR(modbus_slave_ip, cb_type, feeder_type):
    ...
    # Created a shared memory to exchange information
    if feeder_type == 'load':
    ...
    while True:
        ...
        if cb_status_int[0] == 4:
            ...
            if feeder_type == 'utility':
                p_load = shared_memory.SharedMemory(name=modbus_slave_ip_cb_load,create=False, size=10)
                p_pv = shared_memory.SharedMemory(name=modbus_slave_ip_cb_pv,create=False, size=10)
                p_chp = shared_memory.SharedMemory(name=modbus_slave_ip_cb_chp, create=False, size=10)
                p_bess = shared_memory.SharedMemory(name=modbus_slave_ip_cb_bess, create=False, size=10)
                p_load = p_load.buf[1]
                p_pv = p_pv.buf[1]
                p_chp = p_chp.buf[1]
                p_bess = p_bess.buf[1]
                slave_1.set_values('A', P_addr[0], p_load - p_pv - p_chp - p_bess)
            else:
                p = shm.buf[1]
...
if __name__ == "__main__":
    CB2 = Process(target=CB_SIMULATOR, args=(modbus_slave_ip_cb_load, 'MTZ2', 'load'))
    CB3 = Process(target=CB_SIMULATOR, args=(modbus_slave_ip_cb_pv, 'MTZ2','pv'))
    CB4 = Process(target=CB_SIMULATOR, args=(modbus_slave_ip_cb_bess, 'MTZ2','bess'))
    CB5 = Process(target=CB_SIMULATOR, args=(modbus_slave_ip_cb_chp, 'MTZ2','chp'))
    CB6 = Process(target=CB_SIMULATOR, args=(modbus_slave_ip_cb_ev, 'NSX','ev'))
    CB2.start()
    CB3.start()
    CB4.start()
    CB5.start()
    CB6.start()
    time.sleep(1)
    CB1 = Process(target=CB_SIMULATOR, args=(modbus_slave_ip_cb_utility, 'MTZ2', 'utility'))
    CB1.start()